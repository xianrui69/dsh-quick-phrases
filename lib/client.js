window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * v0.7.3: 相对锚点定位 —— barPos 改为相对偏移量，跟随输入框移动。
		 * 监听锚点变化（ResizeObserver + MutationObserver + 定时器后备），自动调整 bar 位置。
		 * 拖拽时更新偏移量，管理面板仍在 bar 上方，打开/关闭不跳动。
		 */
		function QuickPhrasesBar(props) {
			...
			const computeBarPosition = () => {
				...
				if (anchor === null) {
					return keepInViewport(state.barPos ?? defaultBarPos());
				}
				...
				if (state.barPos !== null) {
					return keepInViewport({
						x: anchorRect.left + state.barPos.offsetX,
						y: anchorRect.top + state.barPos.offsetY
					});
				}
				const defaultPos = defaultBarPos();
				return keepInViewport(defaultPos);
			};
			/**
			 * 挂载时计算初始位置，并设置监听器。
			 */
			react.useEffect(() => {
				const initialPos = computeBarPosition();
				setPos(initialPos);
				...
				const updatePosition = () => {
					const newPos = computeBarPosition();
					setPos(newPos);
				};
				try {
			...
			const onContainerPointerUp = (event) => {
				...
				const anchor = anchorRef.current;
				if (anchor === null) {
					ops.setBarPos({ offsetX: 0, offsetY: 0 });
					return;
				}
				const anchorRect = anchor.getBoundingClientRect();
				ops.setBarPos({
					offsetX: pos.x - anchorRect.left,
					offsetY: pos.y - anchorRect.top
				});
			};
