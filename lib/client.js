window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_PHRASES = [
			{ id: "d-continue", name: "继续", text: "继续上面的工作，从上次中断的地方接着做。", pinned: true, autoSubmit: true },
			{ id: "d-explain", name: "解释", text: "请用通俗的语言解释一下刚才的改动和结论。", pinned: false },
			{ id: "d-review", name: "复查", text: "复查一下你刚才的方案/改动，指出潜在问题并给出改进建议。", pinned: false },
			{ id: "d-test", name: "测试", text: "为刚才的改动补充必要的测试并运行验证，把结果汇总给我。", pinned: false },
			{ id: "d-commit", name: "提交", text: "把当前改动整理成一次 git 提交，先给我看拟好的 commit message 再执行。", pinned: false },
			{ id: "d-daily", name: "日报", text: "根据今天的会话记录，帮我整理一份今日工作日报（做了什么、结果如何、待办）。", pinned: false }
		];
		...
		/**
		 * The `/` trigger source: typing `/` lists phrases (置顶优先), picking one
		 * replaces the token span with the phrase text via the plain-text path.
		 * `matchEnter` additionally expands `/名称` + Enter inside the submit
		 * attempt (the input machine applies the returned {text} outcome).
		 */
		const source = {
			trigger: "/",
			name: "短语",
			...
			async candidates(_session, { query, signal }) {
				...
				const usable = store.get().phrases.filter((phrase) => phrase.text.trim() !== "");
			...
			return react_jsx_runtime.jsx("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx("div", { className: "qp-panel-foot", children: [
						...
						react_jsx_runtime.jsx("span", { className: "qp-hint", children: "输入 / 唤出菜单 · ★置顶排最前 · ➤点击即发送 · /名称+回车 展开并发送" })
					] })
				]
			});
		}
		...
		function QuickPhrasesBar(props) {
			...
			if (state.barVisible !== true) return null;
			const chips = state.phrases.filter((phrase) => phrase.text.trim() !== "");
			return react_jsx_runtime.jsxs("div", { className: "qp-bar", children: [
