window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Narrow an unknown persisted panel position to {x,y}, or null. */
		function sanitizePanel(raw) {
		...
		function loadState() {
			try {
				...
				return { version: 1, barVisible: parsed?.barVisible !== false, panel: sanitizePanel(parsed?.panel), barPos: sanitizePanel(parsed?.barPos), phrases };
			} catch {
				return defaultState();
			}
		}
		...
		async function hydrateFromHost() {
			...
			if (body?.missing === true) {
				...
				const validated = seed === null ? defaultState() : {
					...
					panel: sanitizePanel(seed.panel),
					barPos: sanitizePanel(seed.barPos),
			...
			if (Array.isArray(body?.phrases)) store.replace({
				...
				panel: sanitizePanel(body.panel),
				barPos: sanitizePanel(body.barPos),
		...
		const ops = {
			...
			setPanelPos(pos) {
				store.update((draft) => {
					draft.panel = { x: pos.x, y: pos.y };
				});
			},
			setBarPos(pos) {
				store.update((draft) => {
					draft.barPos = { x: pos.x, y: pos.y };
				});
			},
			move(id, delta) {
				store.update((draft) => {
		...
		/**
		 * The floating manage window (v0.5.0, 学 Token 面板): draggable by its
		 * title bar, position remembered in the store (host file + localStorage).
		 * One window serves every entrance — bar ⚙ button, collapsed gear, and
		 * the settings section. Escape still closes; outside clicks do NOT (it's
		 * a window now — the user may want to poke the page behind it).
		 */
		...
		const defaultPanelPos = () => keepInViewport({
			x: Math.round((window.innerWidth - 560) / 2),
			y: 96
		});
		function ManageWindow({ onClose }) {
			const saved = usePhrases().panel;
			const [pos, setPos] = react.useState(() => keepInViewport(saved ?? defaultPanelPos()));
		...
		function QuickPhrasesSettingsSection() {
			...
			return react_jsx_runtime.jsxs("div", { className: "qp-settings-section", children: [
				...
				open && react_jsx_runtime.jsx(ManageWindow, { onClose: () => setOpen(false) })
			] });
		}
		/**
		...
		/**
		 * v0.7.0: 浮动可拖拽的 chips bar（学 ManageWindow）—— fixed 定位、拖拽整个条子、
		 * 位置持久化到 store.barPos。首次加载默认底部居中，记住后就在原处。
		 */
		function QuickPhrasesBar(props) {
			const state = usePhrases();
			const [managing, setManaging] = react.useState(false);
			const saved = state.barPos;
			const [pos, setPos] = react.useState(() => keepInViewport(saved ?? defaultBarPos()));
			const dragRef = react.useRef(null);
			const openManage = () => setManaging(true);
			/** 拖拽处理器（同 ManageWindow，pointer 事件 + capture 模式）。*/
