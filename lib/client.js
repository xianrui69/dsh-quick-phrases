window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * v0.7.5: 「点击别处关闭」的热区判定 —— 整个浮动单元（`.qp-bar-container`
		 * 里的管理面板 + chips 条）都算内部，只有落在单元之外才关闭。
		 * 教训同 mega menu 空白热区误关闭：存活判断要绑整壳，不能只绑面板本体，
		 * 否则拖条子 / 点 chip 这些「同一单元内的正常操作」会被误判为离开。
		 * 非元素目标（document、文本节点）或缺 closest 的桩对象一律按外部处理。
		 */
		const isInsideFloatingUnit = (target) => {
			if (target === null || target === void 0) return false;
			if (typeof target.closest !== "function") return false;
			return target.closest(".qp-bar-container") !== null;
		};
		/**
		 ...
		 * v0.7.5 additionally closes on any pointerdown outside the bar unit.
		 */
		function ManageWindow({ onClose }) {
			...
			react.useEffect(() => {
				/** pointerdown（不是 click）：拖拽条子结束时补发的那一下 click 不会误关。 */
				const onPointerDown = (event) => {
					if (isInsideFloatingUnit(event.target)) return;
					onClose();
				};
		...
		/** Test/debug hook: v0.7.5 click-outside hot-zone predicate. */
		exports.__isInsideFloatingUnit = isInsideFloatingUnit;
