window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Narrow an unknown persisted bar position to {x,y}, or null. */
		...
		function loadState() {
			try {
				...
				return { version: 2, barVisible: parsed?.barVisible !== false, barPos: sanitizeBarPos(parsed?.barPos), phrases };
			} catch {
				return defaultState();
			}
		}
		...
		/**
		 * v0.7.1: 浮动可拖拽的 chips bar + 附加管理面板 —— 作为一个整体移动。
		 * 容器使用 fixed 定位、拖拽整个单元、位置持久化到 store.barPos。
		 * 管理面板不再是独立的浮动窗口，而是附加在 bar 上方，跟随 bar 一起移动。
		 */
